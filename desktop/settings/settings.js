...
const translations = {
  zh:{
    ...
    apiTitle:"API", apiDescription:"推荐 · 无需安装命令行工具", recommended:"推荐", kimiDescription:"官方合作伙伴 · Kimi K3", codexDescription:"自动安装或使用已有账户", claudeDescription:"自动安装或使用已有账户",
    ...
    codexRequired:"通过 Codex CLI 使用你现有的 ChatGPT 登录状态。", codexInstallHelp:"PenEcho 会安装官方 CLI，并直接使用你已有的登录状态。", claudeRequired:"通过 Claude Code 使用你现有的 Anthropic 登录状态。", claudeInstallHelp:"PenEcho 会安装官方稳定版 CLI，并直接使用你已有的登录状态。", install:"安装", modelOptional:"模型（可选）", pathOptional:"可执行文件路径（可选）",
...
function payload() {
  return {
    ...
    kimiProduct:value("kimiProduct"), kimiRegion:value("kimiRegion"),
...
async function initialize() {
  ...
  try {
    ...
    for (const name of ["apiFormat","apiUrl","apiModel","kimiProduct","kimiRegion","codexModel","codexPath","claudeModel","claudePath","effort","imageFormat","timeout","autoDelay","host","port","traceLimit"]) assign(name, settings[name]);
...
async function testAndLaunch() {
  ...
  try {
    ...
    if (!result.ok) {
      const selected = provider(), missingCli = ["codex-cli", "claude-cli"].includes(selected) && /not found|executable|install/i.test(result.error || ""),
        action = missingCli
...
async function installProvider(providerName) {
  ...
  try {
    ...
    assign(providerName === "codex-cli" ? "codexPath" : "claudePath", result.executable);
