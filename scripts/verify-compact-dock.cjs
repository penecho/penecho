...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 1100, 900, 760, 701]) {
      ...
      const expected = { 1440: 'horizontal', 900: 'vertical', 701: 'rows' }[width];
      if (expected) assert.equal(layout.mode, expected);
      ...
      close(layout.mode === 'vertical' ? layout.zoom.x : layout.lock.x, layout.space.x, 'view anchors to left edge');
      close((layout.dock.x + layout.dock.right) / 2, (layout.space.x + layout.space.right) / 2, 'drawing dock stays centered');
      const viewCenter = (layout.lock.y + layout.zoom.bottom) / 2;
