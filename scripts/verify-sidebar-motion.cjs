...
const pause=ms=>new Promise(r=>setTimeout(r,ms)),cpuThrottle=Number(process.env.MOTION_CPU||4);let win;
...
app.whenReady().then(async()=>{try{
 ...
 await partition.protocol.handle('http',async request=>{
  ...
  if(fs.existsSync(filename)&&fs.statSync(filename).isFile()){
   let bytes=process.env.MOTION_SOURCE==='baseline'&&['/app.js','/studio-shell.js','/studio-shell.css','/style.css'].includes(u.pathname)?require('node:child_process').execFileSync('git',['show','7f1a80f:public'+u.pathname],{cwd:root,maxBuffer:32*1024*1024}):fs.readFileSync(filename);
 ...
 if(process.env.MOTION_SOURCE!=='baseline'){
  ...
  assert.equal(await js(`(()=>{const frame=document.querySelector('.canvas-frame');frame.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}));return [...frame.getAnimations({subtree:true})].filter(a=>a.playState==='running'&&a.effect?.target?.id==='viewport').length;})()`),0,'Canvas gestures finish viewport movement before reading coordinates');
  await pause(300);
 ...
 const work=await js(`({fit:window.motionFitTimes,dock:window.motionDockTimes,nodes:document.querySelectorAll("*").length})`);const report={label,cpuThrottle,runs,geometry,errors,work};fs.writeFileSync(path.join(out,label+'.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({...report,runs:runs.map(({positions,...r})=>r)},null,2));
