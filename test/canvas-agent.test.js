...
test("Canvas Agent mounts the minimal project tools only for a host-resolved project",async t=>{
  ...
  for(const name of ["edit","load_project_plugin","read","read_image","write"])assert.equal(visible.includes(name),true,name);
  const bashAvailable=visible.includes("bash");
  assert.equal(visible.includes("list_directory"),!bashAvailable,"hosts without a fully confined Bash runner need bounded folder discovery");
  ...
  const signal=new AbortController().signal,loaded=await host.context.tools.execute({callId:"load-documents",name:"load_project_plugin",arguments:{plugin:"documents"},agent:session.handle.agent,signal});
  ...
  if(bashAvailable){
    const inside=await host.context.tools.execute({callId:"read-inside-bash",name:"bash",arguments:{command:"cat inside.txt"},agent:session.handle.agent,signal});
    assert.equal(inside.isError,false,JSON.stringify(inside));
    assert.match(inside.content[0].text,/inside/);
    const outside=await host.context.tools.execute({callId:"read-outside-bash",name:"bash",arguments:{command:`cat ${JSON.stringify(outsidePath)}`},agent:session.handle.agent,signal});
    assert.equal(outside.isError,false,JSON.stringify(outside));
    assert.doesNotMatch(outside.content[0].text,/outside secret/);
    assert.match(outside.content[0].text,/sandbox denied|operation not permitted|permission denied|no such file|exit code/i);
    const rejected=await host.context.tools.execute({callId:"critical-bash",name:"bash",arguments:{command:"rm inside.txt"},agent:session.handle.agent,signal});
    assert.equal(rejected.isError,true);
    assert.match(rejected.content[0].text,/did not authorize/);
  }else{
  ...
  assert.notEqual(switched.id,session.id,"changing access mode must create a new capability session");
});
...
test("Canvas Agent UI and browser Facade support local and Cloud runtimes and are revision guarded",()=>{
  ...
  assert.match(functionSource(source,"canvasAgentAddProject"),/pickerToken:picked\.pickerToken/);
  assert.match(functionSource(source,"canvasAgentAddProjectFile"),/pickerToken:picked\.pickerToken/);
  assert.doesNotMatch(functionSource(source,"canvasAgentAddProject"),/origin:"desktop"/);
  ...
  assert.match(functionSource(source,"canvasAgentRenderProjects"),/project\.kind==="file"[\s\S]*?canvasAgentProjectAccess\.hidden=!selectedFolder/);
  assert.match(functionSource(source,"canvasAgentRenderProjects"),/sessionProjectId===selected\?\.id[\s\S]*?canvasAgentFolderNoBashHelp/);
  ...
  assert.match(source,/\/api\/canvas-agent\/roots[\s\S]*?\/entries\?path=[\s\S]*?\/api\/canvas-agent\/projects\/from-root/);
  ...
  assert.match(functionSource(source,"canvasAgentSetAccessMode"),/canvasAgentResolveApproval\(false\)[\s\S]*?canvasAgentPersistCurrentConversation\(\)/);
  ...
  assert.match(runtime,/--unshare-net/);
  ...
  assert.match(css,/@media \(pointer: coarse\)[\s\S]*?\.canvas-agent-project-remove[\s\S]*?\.canvas-agent-project-access button[\s\S]*?min-height: 44px/);
