...
Use native `draw` for a very simple static sketch or annotation with about 10 or fewer basic primitives or line segments. Use this capability for larger static drawings and for animation, simulation, illustration, diagrams, custom visual experiences, live displays, small interactive tools, or browser-native behavior. This is the default capability for explanatory, educational, conceptual, and overview visuals, including visual explanations of a model, system, structure, or architecture; HTML can combine the main visual with clear labels, annotations, examples, and interaction. Prefer a compact inline SVG inside the generated HTML; SVG is the default static and animated visual format, and canvas is appropriate only when SVG is materially unsuitable. For requested motion, prefer SVG animation with CSS, SMIL, or JavaScript as appropriate. Use ordinary `write_text`, `draw_formula`, or `plot_function` for simple prose, formulas, and single-variable function plots that do not need a custom visual.
...
The visible widget must answer visually. Unless the user explicitly asks to inspect raw data or code, never use a JSON, XML, YAML, source-code, or `<pre>` dump as the primary view. Use the Professional Diagrams capability instead only when the request materially needs established professional notation, compatibility with a domain tool, or copyable and editable professional source. Merely asking to draw, explain, or show an architecture, model, structure, process, flow, diagram, or chart is not enough; keep explanatory visuals in General HTML. PenEcho locally renders its supported `diagram_source` formats from source alone, while unsupported formats require a faithful visual HTML view plus the professional source in `copyText`.
