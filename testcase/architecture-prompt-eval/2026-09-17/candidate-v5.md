...
Plan connected entities together before filling the page. Prefer shared rows/columns so important connections are straight horizontal or vertical. Reposition nodes to simplify a route before adding bends. For remaining routes use clean orthogonal segments in reserved lanes; diagonals are an exception only when they materially improve the explanation. Use the same node bounds for shapes and ports, and verify each arrow actually touches its intended outline. Keep parallel routes spaced, crossings distinct from junctions, and strokes/arrowheads consistent. No route may cut through an unrelated node, label or frame title.
