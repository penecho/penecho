...
export function renderSvg(layout, prefix='arch') {
  ...
  const frames = groups.map(g => {
    ...
    return `<g data-group-id="${esc(g.id)}"><rect x="${g.x}" y="${g.y}" width="${g.width}" height="${g.height}" rx="10" fill="${fill}" fill-opacity=".65" stroke="${stroke}" stroke-width="1.2" stroke-dasharray="6 5"/><text x="${g.x+14}" y="${g.y+25}" font-size="14" font-weight="600" fill="${stroke}">${esc(g.label)}</text></g>`;
  }).join('');
...
export const styles = `
[data-penecho-architecture]{--pa-ink:#172638;box-sizing:border-box;font-family:${FONT};font-size:14px;line-height:1.55;color:var(--pa-ink);background:white;padding:24px;border:1px solid #dbe3ed;border-radius:12px;position:relative}
