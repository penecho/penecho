...
const DEBUG_TOOLS = new Set(["write_text", "draw_formula", "plot_function", "animate_scene", "html_widget", "diagram_source", "erase"]),
    ...
    try {
      ...
      const effort=providerEffort(payload.reasoningEffort),modelInput = {
        ...
        actionMeaning:{
          ...
          plot:"produce at least one renderable visual command; use plot_function for y=f(x), otherwise use General HTML with inline SVG",
      ...
      if(payload.userAction!=="normalize"&&(invalidTextLayout||manualEmpty||plotMissing)){
        ...
        const retry=plotMissing?"Perform a second independent inspection using focusInset for transcription if available. The user explicitly selected plot. Return at least one renderable visual command. For a single-variable function, return plot_function with an ASCII expression using explicit multiplication such as 3*x. For other requested visuals, return one General HTML html_widget with inline SVG. Do not answer with prose or draw_formula alone.":manualEmpty?MANUAL_EMPTY_RETRY:REINSPECTION_RETRY;
