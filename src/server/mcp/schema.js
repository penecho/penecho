...
const validators = {
  ...
  penecho_present_widget(input) {
    object(input, "arguments");
    exactKeys(input, new Set(["sessionId", "artifactId", "title", "html", "architecture", "sequence", "width", "height", "capture", "quality", "presentation"]), "arguments");
    if ([input.html,input.architecture,input.sequence].filter(value => value !== undefined).length !== 1) invalid("Provide exactly one of html, architecture or sequence. Load the matching guidance for semantic diagram fields.");
    let html;
    if (input.sequence !== undefined) {
      try { html = require("../../sequence/schema.js").sequenceHtml(input.sequence); }
      catch (error) { invalid(error.message); }
    } else if (input.architecture !== undefined) {
...
const TOOLS = [
  {name:"penecho_inbox",description:"Read independent message/feedback pages; preserve both cursors. Only mode:ack marks IDs. Screenshots are opt-in.",inputSchema:{type:"object",additionalProperties:false,required:["sessionId"],properties:{sessionId:{type:"string",minLength:1,maxLength:128},mode:{type:"string",enum:["read","ack"],default:"read"},messageAfter:{type:"integer",minimum:0,default:0},feedbackAfter:{type:"integer",minimum:0},limit:{type:"integer",minimum:1,maximum:50,default:10},capture:{type:"boolean",default:false},quality:{type:"string",enum:["basic","detail"]},ids:{type:"array",minItems:1,maxItems:50,uniqueItems:true,items:{type:"string",minLength:1,maxLength:128}},status:{type:"string",enum:[...MESSAGE_STATUSES]},message:{type:"string",maxLength:1000}},allOf:[{if:{properties:{mode:{const:"ack"}},required:["mode"]},then:{required:["ids","status"],properties:{messageAfter:false,feedbackAfter:false,limit:false,capture:false,quality:false}},else:{properties:{ids:false,status:false,message:false}}},{if:{required:["quality"]},then:{required:["capture"],properties:{capture:{const:true}}}}]}},
  {
    ...
    description:"Read task-specific authoring guidance. Visual Explorer includes its complete base contract and a scoped-rule catalog, not rule bodies. Load a matching rule separately (architecture or sequence for their respective diagrams); rules apply only to that region/current artifact. Other guides are brief by default, full for examples. Reuse unchanged version/hash.",
  ...
  },
  {
    ...
    inputSchema:{ type:"object", additionalProperties:false, required:["sessionId","artifactId","title"], oneOf:[{required:["html"],not:{anyOf:[{required:["architecture"]},{required:["sequence"]}]}},{required:["architecture"],not:{anyOf:[{required:["html"]},{required:["sequence"]}]}},{required:["sequence"],not:{anyOf:[{required:["html"]},{required:["architecture"]}]}}], properties:{sessionId:{type:"string",minLength:1,maxLength:128},artifactId:{type:"string",minLength:1,maxLength:128},title:{type:"string",minLength:1,maxLength:MAX_TITLE_CHARS},html:{type:"string",minLength:1,maxLength:MAX_HTML_CHARS},architecture:{type:"object",description:"Semantic architecture/1 JSON for local rendering. Load architecture guidance for fields; omit html."},sequence:{type:"object",description:"Semantic sequence/1 JSON for local rendering. Load sequence guidance for fields; omit html and architecture."},width:{type:"number",minimum:300,maximum:4096,description:"CSS width: capped on creation, exact for inspect."},height:{type:"number",minimum:200,maximum:4096,description:"CSS height: independently capped on creation, exact for inspect."},capture:{type:"boolean",default:false},quality:{type:"string",enum:["basic","detail"]},presentation:presentationSchema({allowInspect:true})}, allOf:[{if:{required:["quality"]},then:{required:["capture"],properties:{capture:{const:true}}}},{if:{properties:{presentation:{properties:{intent:{const:"inspect"}},required:["intent"]}},required:["presentation"]},then:{required:["capture"],properties:{capture:{const:true}}}},{if:{properties:{presentation:{required:["size"]}},required:["presentation"]},then:{not:{anyOf:[{required:["width"]},{required:["height"]}]}}}] },
  },
