...
const { DEFAULT_MAX_TOKENS, MIN_MAX_TOKENS, configuredMaxTokens, NOVITA_ENDPOINTS, NOVITA_MODELS } = require("../server/api-config.js");
...
const NOVITA_API_PRESETS = Object.freeze([
  Object.freeze({ value:"novita-openai", name:"Novita - Chat Completions", format:"openai", baseUrl:NOVITA_ENDPOINTS.openaiBaseUrl }),
  Object.freeze({ value:"novita-anthropic", name:"Novita - Messages", format:"anthropic", baseUrl:NOVITA_ENDPOINTS.anthropicBaseUrl }),
]);
...
function currentNovitaApiPreset(format, url) {
  const normalizedUrl = cleanText(url).replace(/\/+$/, "");
  return NOVITA_API_PRESETS.find(preset => (!format || preset.format === format) && preset.baseUrl === normalizedUrl) || null;
}

function novitaModelDescription(model) {
...
async function chooseNovitaModel(ui, current) {
  const models = Object.values(NOVITA_MODELS), choices = [
    ...models.map(model => ({ name:model.modelId, value:model.modelId, description:novitaModelDescription(model) })),
    ...(current && !NOVITA_MODELS[current] ? [{ name:`Current custom model: ${current}`, value:current }] : []),
    { name:"Enter a model manually…", value:"__manual__", description:"Use another Novita model ID." },
  ];
...
async function configureApi(ui, configuration, options) {
  ...
  const env = configuration.env, currentFormat = apiValue(env, "FORMAT").toLowerCase(),
    ...
    currentPreset = currentNovitaApiPreset(currentFormat, currentUrl),
    selectedType = await ui.select("API type", [
      ...
      ...NOVITA_API_PRESETS.map(preset => ({ name:preset.name, value:preset.value, description:`Uses ${preset.baseUrl}.` })),
    ], currentPreset?.value || (["openai","anthropic"].includes(currentFormat) ? currentFormat : "openai")),
    preset = NOVITA_API_PRESETS.find(candidate => candidate.value === selectedType) || null,
    ...
    model = preset ? await chooseNovitaModel(ui, sameSelection ? currentModel : "") : cleanText(await ui.input("Model", defaultModel, textValidator("Model"))),
    modelMetadata = NOVITA_MODELS[model] || null,
