...
export function renderSvg(layout,prefix='seq') {
  ...
  const frameTitles=fragments.map(f => `<g data-fragment-label="" style="paint-order:stroke;stroke:white;stroke-width:3px;stroke-linejoin:round">${lines(f.titleLines,f.titleX,f.y+20,{color:'#39516d',weight:600,anchor:'start'})}${f.branches.map(b => `<line x1="${f.x}" x2="${f.x+f.width}" y1="${b.y}" y2="${b.y}" stroke="#a1afbf" stroke-dasharray="6 4"/>${lines(b.lines,f.titleX,b.y+19,{color:'#39516d',anchor:'start'})}`).join('')}</g>`).join('');
...
export function renderContent(layout,prefix,copy) { return renderPanel(layout.data,renderSvg(layout,prefix),copy); }
