...
- **General HTML ウィジェットで公開データをリアルタイム表示。** 公開 HTTPS API、RSS、画像がブラウザの CORS によって遮断された場合、PenEcho のローカル読み取り専用ブリッジへフォールバックし、認証情報を公開せずにニュース、ダッシュボード、その他の更新コンテンツを表示できます。
- **アニメーションと複雑なグラフィックは SVG を優先。** アニメーションや複雑なカスタムビジュアルは General HTML 内のレスポンシブ SVG を標準とし、モデル出力をコンパクトかつトークン効率よく保ちながら、より豊かな動き、オーバーレイ、拡縮可能な表現を実現します。
...
- **プラグインの既定値と軽量な Prompt。** **General HTML** は常に有効で、チェックを外すことはできません。**Professional Diagrams** は既定で有効ですが無効にできます。その他の内蔵・非公開プラグインはすべて既定で無効です。明示的に保存した選択はアップグレード後も維持されます。有効なプラグインの簡潔な機能ガイドだけをモデルに送り、完全な CSS と描画依存関係はローカルで必要時に読み込みます。
...
- **実写画像とプロ向けフローチャートを内蔵。** Real Photo Search は出典付きの Web 画像をキャンバスに直接表示し、既定では 1 件、主画像の読み込みに失敗した場合は代替ソースを使えます。Flowchart はプロセス、判断、アーキテクチャ、シーケンス、状態図を生成し、Mermaid ソースをコピーできます。
