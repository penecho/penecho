...
- **General HTML 위젯의 실시간 공개 데이터.** 공개 HTTPS API, RSS 피드 또는 이미지가 브라우저 CORS에 의해 차단되면 PenEcho의 로컬 읽기 전용 브리지로 대체하여 자격 증명을 노출하지 않고 뉴스, 대시보드 및 기타 갱신 콘텐츠를 표시할 수 있습니다.
- **애니메이션과 복잡한 그래픽에 SVG 우선 적용.** 애니메이션과 복잡한 사용자 지정 시각화는 이제 General HTML 안의 반응형 SVG를 기본으로 사용하여 모델 출력을 간결하고 토큰 효율적으로 유지하면서 더 풍부한 동작, 오버레이 및 확장 가능한 그래픽을 제공합니다.
...
- **가벼운 Prompt를 위한 플러그인 기본값.** **General HTML**은 항상 켜져 있으며 선택을 해제할 수 없습니다. **Professional Diagrams**는 기본으로 켜지지만 끌 수 있고, 그 밖의 모든 내장 및 비공개 플러그인은 기본으로 꺼집니다. 명시적으로 저장한 선택은 업그레이드 후에도 유지됩니다. 활성화된 플러그인의 간결한 기능 안내만 모델에 보내며 전체 CSS와 렌더러 의존성은 로컬에서 필요할 때만 불러옵니다.
...
- **실제 사진과 전문 플로차트 내장.** Real Photo Search는 출처가 있는 웹 이미지를 캔버스에 직접 표시하며, 기본적으로 한 장을 보여 주고 기본 이미지가 실패하면 대체 소스를 사용할 수 있습니다. Flowchart는 프로세스, 의사 결정, 아키텍처, 시퀀스, 상태 다이어그램을 만들고 Mermaid 소스를 복사할 수 있습니다.
