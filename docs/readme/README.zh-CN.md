...
- **通用 HTML 控件支持实时公开数据。** 当公开 HTTPS API、RSS 或图片受到浏览器 CORS 限制时，控件可回退到 PenEcho 的本地只读桥接，动态展示新闻、仪表盘及其他定时刷新内容，同时无需暴露密钥。
- **动画与复杂图形优先使用 SVG。** 动画和复杂自定义图形现在默认通过通用 HTML 中的响应式 SVG 实现，以紧凑、可缩放的矢量描述提供更丰富的动态、叠加与视觉表现，并有效利用模型 token。

## 0.8.0 新功能

- **不止流程图的专业图示。** 可生成架构、UML、时序、BPMN、数据、工程、科学、医学、金融、地理等专业图示，并保留可编辑的专业源码。已支持的格式按需本地渲染；更专门的格式可继续用完整 HTML 展示。
- **插件默认设置与更小的 Prompt 开销。** **General HTML（通用 HTML）**始终开启且不可取消；**Professional Diagrams（专业图示）**默认开启但允许关闭；其余所有内置或私有插件均默认关闭。升级时会保留用户已经明确保存的选择。只向模型发送已启用插件的精简能力说明；完整 CSS 与渲染依赖保留在本地并按需加载。
