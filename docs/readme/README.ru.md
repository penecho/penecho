...
- **Актуальные публичные данные в виджетах General HTML.** Если публичный HTTPS API, RSS-канал или изображение блокируется браузерной политикой CORS, виджет может использовать локальный канал PenEcho только для чтения и показывать новости, панели и другое обновляемое содержимое без раскрытия учётных данных.
- **SVG по умолчанию для анимации и сложной графики.** Анимации и сложные пользовательские визуализации теперь в первую очередь создаются как адаптивный SVG внутри General HTML, обеспечивая более выразительное движение, наложения и масштабируемую графику при компактном и эффективном по токенам выводе модели.

## Что нового в 0.8.0

- **Профессиональные диаграммы, а не только блок-схемы.** Создавайте архитектурные, UML, последовательностные, BPMN, инженерные, научные, медицинские, финансовые и географические диаграммы с редактируемым профессиональным исходным кодом. Поддерживаемые форматы визуализируются локально по требованию, а специализированные могут оставаться полноценным HTML.
- **Настройки плагинов по умолчанию с меньшим prompt.** **General HTML** всегда включён, и снять его выбор нельзя. **Professional Diagrams** включён по умолчанию, но его можно отключить; все остальные встроенные и частные плагины по умолчанию выключены. Явно сохранённый выбор сохраняется после обновления. Модели отправляются только краткие описания включённых плагинов; полный CSS и средства визуализации остаются локальными и загружаются по необходимости.
- **Улучшение виджетов плагинов с помощью рисунка.** Нарисуйте или напишите требуемые изменения пером прямо поверх виджета, возвращённого плагином, а затем нажмите появившуюся кнопку **AI Refine**, чтобы создать улучшенную замену. Этот процесс применяется только к виджетам, возвращённым плагинами.
- **Хранение на этом устройстве или сервере PenEcho.** Сохраняйте холст только в текущем браузере либо на компьютере с PenEcho, чтобы его могли открыть другие авторизованные устройства того же сервера. Перед сохранением незавершенные элементы автоматически подтверждаются.
- **Буфер обмена, текст и расширяемые плагины.** Добавляйте текст или изображения из системного буфера, копируйте текст, формулы и профессиональный исходный код от ИИ и повторно редактируйте Textbox инструментом Hand. Встроенный или частный плагин можно скопировать как пользовательский с необязательным CSS, загружаемым только при использовании.
