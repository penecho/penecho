...
- **Включено по умолчанию с меньшим prompt.** **Professional Diagrams** доступен сразу и может быть отключён в **Plugins**. Модели отправляется только краткое описание возможностей; полный CSS и средства визуализации остаются локальными и загружаются по необходимости.
