...
| `TAVILY_API_KEY` | Optional Tavily credential for Canvas Agent internet search; saved locally and used only by the server when the per-device search toggle is on |
