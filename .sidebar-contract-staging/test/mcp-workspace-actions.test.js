...
function harness({documents=[],dirty=false}={}){
  const records=new Map(documents.map(doc=>[doc.id,doc])),calls=[],closed=[],reports=[],controls={showError:null,showGate:null,showAfter:null,lastShowOptions:null,closeError:null,closeGate:null,addOnClose:null};
  ...
  const context=vm.createContext({
    ...
    canvasDocumentsShow:async (id,execution,options)=>{controls.lastShowOptions=options||null;calls.push(["show",id]);const error=typeof controls.showError==="function"?controls.showError(id):controls.showError;if(error)throw error;if(controls.showGate&&controls.showGate.id===id)await controls.showGate.promise;if(options?.markSeen!==false&&records.get(id))records.get(id).unseen=0;canvasDocuments.activeId=id;controls.showAfter?.(id);return true;},
...
function canvasRowCloseHarness({activeId="current",show=async()=>{},unseen=1}={}){
  const calls=[],documentRecord={id:"target",unseen},canvasDocuments={activeId,switching:false,records:new Map([[documentRecord.id,documentRecord]])};
  let click,transition;
  ...
  const context=vm.createContext({
    ...
    canvasDocumentsShow:async (id,execution,options)=>{calls.push(options===undefined?["show",id]:["show",id,options]);await show(id,options);if(options?.markSeen!==false)documentRecord.unseen=0;canvasDocuments.activeId=id;calls.push(["shown",id]);},
    requestCanvasTransition:value=>{transition=value;calls.push(["transition",value.type,value.documentId,canvasDocuments.activeId]);return false;},
  });
  ...
  return {calls,documentRecord,transition:()=>transition,click:()=>click()};
}
...
test("Canvas row close awaits background activation before requesting the dirty-aware transition",async()=>{
  ...
  assert.deepEqual(h.calls,[["show","target",{markSeen:false}]]);
  release();await closing;
  assert.deepEqual(h.calls,[["show","target",{markSeen:false}],["shown","target"],["transition","close","target","target"]]);
});
...
test("Canvas row close activates a background document without acknowledging unread before confirmation",async()=>{
  const h=canvasRowCloseHarness({activeId:"other",unseen:2});
  await h.click();
  assert.deepEqual(h.calls,[["show","target",{markSeen:false}], ["shown","target"], ["transition","close","target","target"]]);
  assert.equal(h.documentRecord.unseen,2,"a pending dirty close must leave the background update unread");
  assert.equal(h.transition()?.type,"close");
});
...
test("automatic Follow latest preserves unread acknowledgement",async()=>{
  ...
  assert.equal(h.records.get("latest").unseen,1);
});
